...
flash: $(BINARY).flash
gdb-repl: $(BINARY).gdb-repl
pwr-flash: $(BINARY).pwr-flash
pwr-on: $(BINARY).pwr-on
pwr-off: $(BINARY).pwr-off
pwr-repl: $(BINARY).pwr-repl
...
%.write-opt: %.elf
	...
	$(GDB) --batch \
		   ...
		   -ex 'monitor swdp_scan' \
		   -ex 'attach 1' \
		   -ex 'monitor option write 0xFBFEF8AA' \
		   $(*).elf

%.pwr-flash: %.elf
	@printf "  GDB   $(*).elf (pwr flash)\n"
	$(GDB) --batch \
		   -ex 'target extended-remote $(BMP_PORT)' \
...
%.pwr-repl: %.elf
	@printf "  GDB   (pwr on)\n"
	$(GDB)	   -ex 'target extended-remote $(BMP_PORT)' \
		   -ex 'monitor tpwr enable' \
		   -ex 'monitor swdp_scan' \
		   -ex 'attach 1' \
		  $(*).elf
